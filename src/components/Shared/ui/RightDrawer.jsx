...
export const RightDrawer = ({ title, onClose, children, width = 380, borderLeft = true }) => {
  return (
    <div className="overflow-y-auto flex flex-col shrink-0 transition-all duration-300" style={{ width, background: T.card, borderLeft: borderLeft ? `1px solid ${T.bd}` : "none" }}>
    ...
    <div style={{ borderBottom: `1px solid ${T.bdLight}` }}>
      <div className="w-full flex items-center justify-between px-3 py-2 transition-colors cursor-pointer group"
        ...
        <div className="flex items-center gap-1.5">
          {open ? <ChevronDown size={11} style={{ color: T.t4 }} className="group-hover:text-gray-700" /> : <ChevronRight size={11} style={{ color: T.t4 }} className="group-hover:text-gray-700" />}
